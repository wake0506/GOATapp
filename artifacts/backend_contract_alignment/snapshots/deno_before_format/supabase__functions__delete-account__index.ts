...
export function createDeleteAccountHandler(
  ...
  return async (request: Request) => {
    ...
    if (contentType !== 'application/json') return error('INVALID_CONTENT_TYPE', 415, requestId);
    const token = request.headers.get('authorization')?.replace(/^Bearer\s+/i, '').trim() ?? '';
    if (!token) return error('UNAUTHORIZED', 401, requestId);

    const raw = await request.text();
    if (new TextEncoder().encode(raw).byteLength > MAX_BODY_BYTES) return error('REQUEST_TOO_LARGE', 413, requestId);
    let body: Record<string, unknown>;
    try {
      const parsed = JSON.parse(raw) as unknown;
      if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return error('INVALID_BODY', 400, requestId);
      body = parsed as Record<string, unknown>;
    } catch {
      return error('INVALID_BODY', 400, requestId);
    }
    if (body.confirmPhrase !== 'DELETE MY ACCOUNT') return error('CONFIRMATION_REQUIRED', 400, requestId);

    const url = readEnv('SUPABASE_URL') ?? '';
    const anon = readEnv('SUPABASE_ANON_KEY') ?? key(readEnv('SUPABASE_PUBLISHABLE_KEYS'));
    const secret = readEnv('SUPABASE_SERVICE_ROLE_KEY') ?? key(readEnv('SUPABASE_SECRET_KEYS'));
    if (!url || !anon || !secret) return error('CONFIGURATION_ERROR', 500, requestId);

    const userClient = clientFactory(url, anon, { global: { headers: { Authorization: `Bearer ${token}` } } });
    ...
    const admin = clientFactory(url, secret, { auth: { autoRefreshToken: false, persistSession: false } });
    const ready = await admin.rpc('assert_account_deletion_ready');
    if (ready.error || ready.data !== true) {
      console.error(JSON.stringify({ requestId, result: 'CASCADE_CHECK_FAILED' }));
