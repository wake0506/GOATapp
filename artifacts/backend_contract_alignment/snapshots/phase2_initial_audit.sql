...
resolved_functions as (
  select
    function_name,
    pg_catalog.to_regprocedure(function_name) as procedure_oid
  from functions as function_values(function_name)
)
select
  function_row.function_name,
  role_row.role_name,
  case
    when role_row.role_name = 'PUBLIC' then exists (
      select 1
      ...
      role_row.role_name,
      function_row.procedure_oid,
      'EXECUTE'
    )
  end as can_execute
from resolved_functions function_row
join pg_catalog.pg_proc procedure_row
  on procedure_row.oid = function_row.procedure_oid
cross join real_roles as role_row(role_name)
