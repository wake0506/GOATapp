...
function makeClientFactory(options: {
  userId?: string | null;
  rpc?: (name: string, args: Record<string, unknown>) => Promise<MockRpcResponse>;
  keys?: string[];
}) {
  return ((_: string, key: string) => {
    ...
    return {
      auth: {
        getUser: async () => ({
          data: { user: options.userId === null ? null : { id: options.userId ?? 'test-user' } },
          error: null,
        }),
      },
...
function validRequestBody(clientRequestId = 'request-1') {
  return JSON.stringify({ text: '一碗米饭', defaultMealType: '早餐', clientRequestId });
}
...
Deno.test('legacy Supabase keys are preferred', async () => {
  ...
  const handler = createNutritionAiHandler(
    fetch,
    makeClientFactory({ keys, rpc: async () => ({ data: { status: 'IN_PROGRESS' }, error: null }) }),
    testEnv,
  );
  await handler(
...
function makeFlowRpc(options: {
  ...
  const rpc = async (name: string, args: Record<string, unknown>) => {
    ...
    if (name === 'nutrition_ai_claim_operation') {
      return { data: options.claim ?? { status: 'CLAIMED', claimToken: 'claim-token' }, error: null };
    }
...
Deno.test('UTF-8 oversized and invalid JSON requests are rejected', async () => {
  ...
  if (oversized.status !== 413) throw new Error('UTF-8 oversized body was accepted');
...
Deno.test('CLAIMED, IN_PROGRESS and CACHED states are handled', async () => {
  ...
  const inProgress = parseClaimState({ status: 'IN_PROGRESS', claimToken: null });
  const cached = parseClaimState({
    status: 'CACHED',
    claimToken: null,
    response: { items: [validItem], requestId: 'cached-1', provider: 'deepseek' },
  });
  if (claimed.status !== 'CLAIMED' || inProgress.status !== 'IN_PROGRESS' || cached.status !== 'CACHED') {
  ...
  if (inProgressResponse.status !== 409) throw new Error('IN_PROGRESS was not returned as 409');
  ...
  const cachedResult = await makeHandler(
    makeFlowRpc({ claim: { status: 'CACHED', claimToken: null, response: cachedResponse } }).rpc,
  )(
    new Request('https://example.invalid/nutrition-ai', {
      method: 'POST',
      headers: { authorization: 'Bearer test-jwt' },
  ...
  if (cachedResult.status !== 200) throw new Error('CACHED was not returned as 200');
});
...
Deno.test('cached response is schema-validated before reuse', async () => {
  ...
  if (response.status !== 503) throw new Error('invalid cached schema was reused');
});
...
Deno.test('provider timeout releases the matching claim token', async () => {
  ...
  const release = flow.calls.find((call) => call.name === 'nutrition_ai_release_operation');
...
Deno.test('invalid provider response is not saved and logs stay redacted', async () => {
  ...
  try {
    response = await makeHandler(flow.rpc, async () => providerResponse('not-json'))(
      new Request('https://example.invalid/nutrition-ai', {
        method: 'POST',
  ...
  const saved = flow.calls.some((call) => call.name === 'nutrition_ai_save_response');
  const logText = logs.join('\n');
  if (response?.status !== 502 || saved || logText.includes('sensitive meal text') ||
      logText.includes('sensitive-jwt') || logText.includes('provider-test-key')) {
    throw new Error('invalid provider response was saved or sensitive data was logged');
  }
});
